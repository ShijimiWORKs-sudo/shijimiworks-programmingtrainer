...
interface RunnerWorker {
  postMessage(message: CppWorkerRequest): void;
  terminate(): void;
  addEventListener(type: "message", listener: (event: MessageEvent<CppWorkerResponse>) => void): void;
  removeEventListener(type: "message", listener: (event: MessageEvent<CppWorkerResponse>) => void): void;
}

type WorkerFactory = () => RunnerWorker;

function createDefaultWorker(): RunnerWorker {
  return new Worker(new URL("./cpp.worker.ts", import.meta.url), { type: "module" });
}

function nextRequestId() {
  return "cpp-" + crypto.randomUUID();
}
...
export class CppRunner implements LanguageRunner {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "timeout") {
        ...
        return {
          status: "timeout",
          stdout: "",
          stderr: "Execution timed out and the C++ worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "timeout",
        };
      }
      ...
      if (error instanceof Error && error.message === "cancelled") {
        return {
          status: "cancelled",
          stdout: "",
          stderr: "Execution was cancelled and the C++ worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "cancelled",
        };
      }
  ...
  private sendAndWait(
    type: CppWorkerRequest["type"],
    payload: Partial<CppWorkerRequest>,
    timeoutMs: number
  ): Promise<CppWorkerResponse> {
    const id = nextRequestId();
    const message = { id, type, ...payload } as CppWorkerRequest;
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent<CppWorkerResponse>) => {
        if (event.data.id !== id) {
          return;
        }
        cleanup();
