...
interface RunnerWorker {
  postMessage(message: JavaWorkerRequest): void;
  terminate(): void;
  addEventListener(type: "message", listener: (event: MessageEvent<JavaWorkerResponse>) => void): void;
  removeEventListener(type: "message", listener: (event: MessageEvent<JavaWorkerResponse>) => void): void;
}

type WorkerFactory = () => RunnerWorker;

function createDefaultWorker(): RunnerWorker {
  return new Worker(new URL("./java.worker.ts", import.meta.url), { type: "module" });
}

function nextRequestId() {
  return "java-" + crypto.randomUUID();
}
...
export class JavaRunner implements LanguageRunner {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "timeout") {
        ...
        return {
          status: "timeout",
          stdout: "",
          stderr: "Execution timed out and the Java worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "timeout",
        };
      }
      ...
      if (error instanceof Error && error.message === "cancelled") {
        return {
          status: "cancelled",
          stdout: "",
          stderr: "Execution was cancelled and the Java worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "cancelled",
        };
      }
  ...
  private sendAndWait(
    type: JavaWorkerRequest["type"],
    payload: Partial<JavaWorkerRequest>,
    timeoutMs: number
  ): Promise<JavaWorkerResponse> {
    const id = nextRequestId();
    const message = { id, type, ...payload } as JavaWorkerRequest;
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent<JavaWorkerResponse>) => {
        if (event.data.id !== id) {
          return;
        }
        cleanup();
