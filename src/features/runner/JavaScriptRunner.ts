...
interface RunnerWorker {
  postMessage(message: JavaScriptWorkerRequest): void;
  terminate(): void;
  addEventListener(type: "message", listener: (event: MessageEvent<JavaScriptWorkerResponse>) => void): void;
  removeEventListener(type: "message", listener: (event: MessageEvent<JavaScriptWorkerResponse>) => void): void;
}
...
export class JavaScriptRunner implements LanguageRunner {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "timeout") {
        ...
        return {
          status: "timeout",
          stdout: "",
          stderr: "Execution timed out and the JavaScript worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "timeout",
        };
      }
      ...
      if (error instanceof Error && error.message === "cancelled") {
        return {
          status: "cancelled",
          stdout: "",
          stderr: "Execution was cancelled and the JavaScript worker was restarted.",
          durationMs: Math.round(performance.now() - startedAt),
          errorType: "cancelled",
        };
      }
  ...
  private sendAndWait(
    type: JavaScriptWorkerRequest["type"],
    payload: Partial<JavaScriptWorkerRequest>,
    timeoutMs: number
  ): Promise<JavaScriptWorkerResponse> {
    const id = nextRequestId();
    const message = { id, type, ...payload } as JavaScriptWorkerRequest;
    ...
    return new Promise((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent<JavaScriptWorkerResponse>) => {
        if (event.data.id !== id) {
          return;
        }
        cleanup();
